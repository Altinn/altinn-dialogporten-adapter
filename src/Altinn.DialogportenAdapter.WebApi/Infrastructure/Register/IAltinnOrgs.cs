...
internal sealed class AltinnOrgs(IFusionCache cache, IHttpClientFactory clientFactory) : IAltinnOrgs
{
    ...
    private async Task<AltinnOrgData?> FetchAltinnOrgData(CancellationToken ct) =>
        await _clientFactory.CreateClient(Constants.AltinnOrgsClient)
            .GetFromJsonAsync<AltinnOrgData>(Constants.AltinnOrgsUrl, ct);
}
